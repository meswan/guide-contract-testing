...
The pact framework provides a [hotspot=pactPlugin file=1]`plugin` that can be added to the build section of the `pom.xml`. It contains the [hotspot=endpoint file=1]`endpoint` URL for the system microservice and the [hotspot=pactDirectory file=1]`location` at which the pact file will be stored.
