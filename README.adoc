...
`src/test/java/it/io/openliberty/guides/inventory/InventoryPactIT.java`
----
...
Open a new command-line session to publish the generated pact file to the pact broker by using:
[role='command']
```
mvn pact:publish
```
...
The [hotspot=state file=1]`@State` annotation must match the [hotspot=given file=2]`.given()` parameter that was provided in the inventory test class so that pact can identify which test case to run against which endpoint.
...
Run the following commands to remove the pact broker application.
[role='command']
```
