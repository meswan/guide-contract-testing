...
is required to perform end-to-end tests that rely on third-party software such as Selenium, which requires heavy compute time and resources.

Contract testing can be used to bridge the gap among the shortcomings of those testing methodologies. Contract testing is a technique for testing an integration point by isolating each application and checking if the
...
learn how to use the https://docs.pact.io/[Pact^] framework to write contract tests for the inventory application to be verified by
...
The pact framework provides an [hotspot=pactPlugin file=0]`extension` that can be added to the build section of the `pom.xml`. It contains the [hotspot=endpoint file=0]`endpoint` URL for the system microservice and the [hotspot=pactDirectory file=0]`location` at which the pact file is to be stored.
The [hotspot=pactJunit file=0]`pact-jvm-consumer-junit` dependency provides the base test class for use with JUnit to build the unit tests.
...
The `InventoryPactIT` class contains a [hotspot=mockprovider file=1]`mock provider` that mimics the HTTP responses from the system microservice.
The [hotspot=pact file=1]`@Pact` annotation takes the name of the microservice as a parameter that makes it easier to differentiate microservices from one another when there are multiple applications.

The [hotspot=builder file=1]`RequestResponsePact` method defines the minimal expected response for a specific endpoint and is also known as an interaction.
For each interaction, the expected request and the response are registered with the mock service by using the [hotspot=verification file=1]`@PactVerification` annotation.

The test sends a real request to the [hotspot=mockTest file=1]`mock provider`. The mock provider compares the actual request with the expected request and confirms if the comparison is successful.
Finally, the [hotspot=unitTest file=1]`unit test` confirms that the response is correct.
...
A contract between inventory and system microservice is known as a pact. Each pact is a collection of interactions that were defined in the `InventoryPactIT` class.
...
With the pact file generated, open a new terminal to publish it to the pact broker by using:
[role='command']
```
mvn pact:publish
```

When completed, you will see something similar to:
[role="no_copy"]
```
...
Refresh the http://localhost:9292/[pact broker webpage^] to verify that a new entry was added. Note there is no
timestamp in the `Last verified` column as the pact is yet to be verified by the system microservice yet.

You can also see the http://localhost:9292/pacts/provider/System/consumer/Inventory/latest[interactions^] URL for more insights about each interaction that was created.
...
The connection information for the pact broker is provided with the [hotspot=connectionInfo file=1]`@PactBroker` annotation. The dependency also provides a [hotspot=invocation file=1]`JUnit5 Invocation Context Provider` to write pact verification tests that are used with the `@TestTemplate` annotation to generate a test for each of the interactions.
...
The test target is defined in the [hotspot=context file=1]`PactVerificationContext` to point to the running endpoint of the system microservice.

The [hotspot=state file=1]`@State` annotation must match the `.given()` parameter that was provided in the inventory test class so that pact can identify which test case to run against.
...
...
```
When completed, refresh the http://localhost:9292/[pact broker webpage^] to confirm that there is now a timestamp in the `Last verified` column.
