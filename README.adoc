...
The [hotspot=builder file=1]`RequestResponsePact` method defines the minimal expected response for a specific endpoint also known as an interaction.
...
The [hotspot=mockTest file=1]`test` sends a real request to the mock provider. The mock provider compares the actual request with the expected request and confirms if the comparison is successful.
