...
Pact is an open source contract testing tool for testing HTTP requests, responses and message integrations using contract tests.
...
Navigate to http://localhost:9292/[^] to confirm that you can access the pact broker's UI.
...
The pact framework provides a [hotspot=pactPlugin file=0]`plugin` that can be added to the build section of the `pom.xml`. It contains the [hotspot=endpoint file=0]`endpoint` URL for the system microservice and the [hotspot=pactDirectory file=0]`location` at which the pact file will be stored.
...
The [hotspot=pact file=1]`@Pact` annotation takes the name of the microservice as a parameter. This makes it easy to differentiate microservices for one another when there are multiple applications.
...
For each interaction, the expected request and the response are registered with the mock service using the [hotspot=verification file=1]`@PactVerification` annotation.
...
A contract between inventory and system microservice is known as a pact. Each pact is a collection of interactions that have been defined in the `InventoryPactIT` class.
...
A snippet of the interaction page should look similar to:
```
...
The pact file created by the inventory microservice has been successfully verified by the system microservice through the pact broker application. This ensures that responses from the system microservice actually meets the expectation of the inventory microservice.
